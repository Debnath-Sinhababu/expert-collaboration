...
import { Calendar, Clock, IndianRupee, Send, ArrowLeft, AlertCircle } from 'lucide-react'

export default function PublicFreelanceDetail() {
    ...
    const init = async () => {
      try {
        ...
        if (user) {
          ...
          if (role === 'student') {
            router.push(`/student/freelance/${id}`)
            return
          }
        }
        
        // Load freelance project details (public API)
        const data = await api.freelance.getProjectById(id)
  ...
  const handleApplyClick = () => {
    if (!user) {
      ...
      const returnUrl = `/requirements/freelance/${id}`
      ...
      if (role === 'student') {
        ...
        router.push(`/auth/login?returnUrl=${encodeURIComponent(`/requirements/freelance/${id}`)}&message=${encodeURIComponent('Only students can apply to freelance projects')}`)
      }
    }
  }
        ...
        <Card className="bg-white border-2 border-[#D6D6D6] rounded-lg mb-6">
          <CardContent className="p-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-6">
              {project.budget_min && (
                <div>
                  <div className="text-xs text-[#6A6A6A] font-medium mb-1">Budget</div>
                  <div className="text-lg font-semibold text-[#008260]">
                    ₹{project.budget_min}
                    {project.budget_max && project.budget_max !== project.budget_min && `-₹${project.budget_max}`}
                  </div>
                </div>
              )}
              {project.deadline && (
                <div>
                  <div className="text-xs text-[#6A6A6A] font-medium mb-1">Deadline</div>
              ...
              <div>
                <div className="text-xs text-[#6A6A6A] font-medium mb-1">Posted on</div>
                <div className="text-sm font-semibold text-[#000000]">
                  {new Date(project.created_at || Date.now()).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                </div>
              </div>
            </div>
            ...
            {project.required_skills && project.required_skills.length > 0 && (
              <div className="mb-6">
                <h3 className="text-base font-semibold text-[#000000] mb-3">Required Skills</h3>
            ...
            {project.draft_attachment_url && (
              <div className="mb-6">
                <h3 className="text-base font-semibold text-[#000000] mb-2">Draft Attachments</h3>
                <a
                  href={project.draft_attachment_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-[#008260] hover:underline text-sm"
                >
                  {project.draft_attachment_url.split('/').pop() || 'View Attachment'}
                </a>
              </div>
            )}
