...
export type InstitutionDashboardViewer = 'institution' | 'super_admin'

export function InstitutionDashboardClient({
  viewer = 'institution',
  actingInstitutionId
}: {
  viewer?: InstitutionDashboardViewer
  ...
  const loadInstitutionData = async (userId: string) => {
    try {
      let institutionsResponse = null as any
      if (viewer === 'super_admin' && actingInstitutionId) {
        institutionsResponse = await api.institutions.getById(actingInstitutionId)
      } else {
        institutionsResponse = await api.institutions.getByUserId(userId)
      }
