...
export const superAdminApi = {
  ...
  runningProjects: (params?: { page?: number; limit?: number }) =>
    request<PaginatedResponse<any>>(`/api/superadmin/requirements/running-projects?${query({ ...(params || {}), _t: Date.now() })}`),
  runningProjectDetail: (bookingId: string, params?: { page?: number; limit?: number; date_from?: string; date_to?: string }) =>
    request<any>(`/api/superadmin/requirements/running-projects/${encodeURIComponent(bookingId)}?${query({ ...(params || {}), _t: Date.now() })}`),
