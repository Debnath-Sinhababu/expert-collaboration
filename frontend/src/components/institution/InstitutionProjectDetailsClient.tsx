...
export type InstitutionProjectDetailsViewer = 'institution' | 'super_admin'

export function InstitutionProjectDetailsClient({
  viewer = 'institution',
  actingInstitutionId,
  projectId: projectIdFromProps
}: {
  viewer?: InstitutionProjectDetailsViewer
  actingInstitutionId?: string
  projectId?: string
}) {
  const params = useParams()
  const router = useRouter()
  const projectId = projectIdFromProps || (params.projectId as string)
  ...
  const loadInstitutionData = async (userId: string) => {
    try {
      let institutionProfile = null as any
      if (viewer === 'super_admin' && actingInstitutionId) {
        institutionProfile = await api.institutions.getById(actingInstitutionId)
      } else {
        institutionProfile = await api.institutions.getByUserId(userId)
      }
