...
export const api = {
  ...
  experts: {
    ...
    switchToCalxbook: async (id: string) => {
      const headers = await getAuthHeaders()
      const res = await fetch(`${API_BASE_URL}/api/experts/${id}/switch-to-calxbook`, {
        method: 'POST',
        headers
      })
      const json = await res.json().catch(() => ({}))
  ...
  projects: {
    ...
    getCounts: async (params?: { institution_id: string }) => {
      const headers = await getAuthHeaders()
      const query = new URLSearchParams({
        ...(params || {}),
        _t: Date.now().toString(),
      } as any).toString()
      const res = await fetch(`${API_BASE_URL}/api/projects/counts?${query}`, { headers })
      const json = await res.json().catch(() => ({}))
      if (!res.ok) throw new Error(json?.error || 'Failed to load project counts')
      return json
    },
    getById: async (id: string) => {
      const headers = await getAuthHeaders()
